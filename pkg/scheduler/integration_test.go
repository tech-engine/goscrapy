...
func (e *blockingExecutor) Execute(req *core.Request, res core.IResponseWriter) error {
	select {
	case <-e.startBlock:
	case <-req.Ctx.Done():
		return req.Ctx.Err()
...
func TestScheduler_GracefulShutdown(t *testing.T) {
	executor := &blockingExecutor{startBlock: make(chan struct{})}
	sched := New(executor, request.NewPool(), WithWorkers(2))
...
type mockExecutorFunc struct {
	execute func(*core.Request, core.IResponseWriter) error
}

func (m *mockExecutorFunc) Execute(req *core.Request, res core.IResponseWriter) error {
...
func TestWorker_Concurrency(t *testing.T) {
	...
	executor := &mockExecutorFunc{
		execute: func(req *core.Request, res core.IResponseWriter) error {
			mu.Lock()
			active++
			if active > maxActive {
				maxActive = active
	...
	numWorkers := uint16(5)
	sched := New(executor, request.NewPool(), WithWorkers(numWorkers))
